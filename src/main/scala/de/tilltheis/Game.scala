...
class Game private(dimensions: Dimensions, playerNames: Set[String], server: ActorRef) extends Actor with ActorLogging {
...
object Game {
  def props(dimensions: Dimensions, players: Set[String], server: ActorRef): Props = Props(new Game(dimensions, players, server))

  case object StartPlaying
  private case object Tick
  case class PlayerBodies(bodies: Map[String, List[Point]])
  case class PlayerAction(player: String, steeringDirection: Option[Direction])
